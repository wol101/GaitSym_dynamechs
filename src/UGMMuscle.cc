...
// relativeWidth - maximum length range of force production relative to optimalLength (note this value is effectively the half width)
...
{
    double energy;
    double delLen = m_Length - m_lce - m_tendonlength;
...
void UGMMuscle::SetStim(double stim, double timeIncrement)
{
    ...
    {
        m_newObject = false;
        double ext = m_Length - (m_tendonlength + m_lceopt);
        if (ext <= 0)
            m_lce = m_Length - m_tendonlength;
        else // need to partition the stretch between the two elastic components
        {
            ...
            else cerr << "Unrecognized combination of strain models\n";
            if (seext <= 0 || peext <= 0) cerr << "Error calculating extensions\n";
    ...
    seext = MAX(m_Length - m_lce - m_tendonlength, 0);
    switch (m_serialStrainModel)
    {
        case linear:
            fse = m_kse * seext;
    ...
    SetTension(fse); // tension in unit is same as tension in serial elastic element
    ...
    if (m_fce <= (m_fiso*m_fmax*m_act))
    {
        ...
        */
        ...
        if (num >= 0)
        {
            ...
            cerr << "Applying concentric m_vce = 0 fixup to " << m_name << "\n";
        }
        else if (den <= 0)
        {
    ...
    else
    {
        ...
        */
        ...
        if (num >= 0)
        {
            m_vce = 0;
            cerr << "Should never get here\n";
            cerr << "Applying eccentric m_vce = 0 fixup to " << m_name << "\n";;
        }
        else if (den >= 0)
        {
            m_vce = m_vmaxft * m_lceopt;
