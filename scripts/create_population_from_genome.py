#!/usr/bin/python -u

import sys
import string
import array
import os
import re
import time

def CopyFile(theInputFileName, theOutputFileName):
    """Copies the contents of a file"""
    theInput = open(theInputFileName, 'r')
    theData = theInput.read()
    theInput.close()
    theOutput = open(theOutputFileName, 'w')
    theOutput.write(theData)
    theOutput.close()

def StartsWith(theString, thePrefix):
    if theString[0: len(thePrefix)] == thePrefix:
        return 1
    return 0

def EndsWith(theString, theSuffix):
    if theString[len(theString) - len(theSuffix):] == theSuffix:
        return 1
    return 0

def PrintExit(value):
    """exits with error message"""
    sys.stderr.write(str(value) + "\n");
    sys.exit(1)

def WriteTokenList(filename, tokenList):
    theOutput = open(filename, 'w')
    for i in range(0, len(tokenList)):
        theOutput.write(str(tokenList[i]) + "\n")
    theOutput.close()
    

def ConvertToListOfTokens(theBuffer):
    """converts a string into a list of tokens delimited by whitespace"""
    
    charList = list(theBuffer)
    i = 0
    tokenList = []
    while i < len(charList):
        byte = charList[i]
        if byte in string.whitespace: 
            i = i + 1
            continue
        word = ""
        if byte == '"':
            i = i + 1
            byte = charList[i]
            while byte != '"':
                word = word + byte
                i = i + 1
                if i >= len(charList): PrintExit("Unmatched \" error")
                byte = charList[i]
        else:
            while (byte in string.whitespace) == 0:
                word = word + byte
                i = i + 1
                if i >= len(charList): break
                byte = charList[i]
        
        if len(word) > 0: tokenList.append(word)
        i = i + 1
            
    return tokenList        

def ConvertFileToTokens(filename):
    """reads in a file and converts to a list of whitespace delimited tokens"""
    
    input = open(filename, 'r')
    theBuffer = input.read()
    input.close()
    
    return ConvertToListOfTokens(theBuffer)
   
def GetIndexFromTokenList(tokenList, match):
    """returns the index pointing to the value in the tokenList (match + 1)
       raises NoMatch on error"""
    for i in range(0, len(tokenList) - 1):
        if tokenList[i] == match: return i + 1
        
    raise "NoMatch", match

def GetValueFromTokenList(tokenList, match):
    """returns value after the matching token in the tokenList (match + 1)
       raises NoMatch on error"""
    for i in range(0, len(tokenList) - 1):
        if tokenList[i] == match: return tokenList[i + 1]
        
    raise "NoMatch", match

def IsANumber(theString):
    """checks to see whether a string is a valid number"""
    if re.match('([+-]?)(?=\d|\.\d)\d*(\.\d*)?([Ee]([+-]?\d+))?', theString) == None: return 0
    return 1
     

def main():
    usageString = "Usage:\n%s genome population_size population" % sys.argv[0]

    if len(sys.argv) != 4:
        PrintExit(usageString)
    
    genomeFile = sys.argv[1]
    populationSize = int(sys.argv[2])
    populationFile = sys.argv[3]
    
    genomeTokenList = ConvertFileToTokens(genomeFile)
    
    index = 0
    # parse the genome
    if int(genomeTokenList[index]) != -1: PrintExit("Error: parsing %s" % genomeFile)
    index = index + 1
    genomeLength = int(genomeTokenList[index])
    index = index + 1
    gene = []
    lowBound = []
    highBound = []
    sd = []
    for k in range(0, genomeLength):
        gene.append(float(genomeTokenList[index])) # gene
        index = index + 1
        lowBound.append(float(genomeTokenList[index])) # low bound
        index = index + 1
        highBound.append(float(genomeTokenList[index])) # high bound
        index = index + 1
        sd.append(float(genomeTokenList[index])) # SD 
        index = index + 1

    fitness = genomeTokenList[index] # fitness
    index = index + 1
    parent1GenerationNumber = genomeTokenList[index] # parent 1
    index = index + 1
    parent1Rank = genomeTokenList[index]
    index = index + 1
    parent2GenerationNumber = genomeTokenList[index] # parent 2
    index = index + 1
    parent2Rank = genomeTokenList[index]
    index = index + 1
    
    # now write out the population
    
    theOutput = open(populationFile, 'w')
    theOutput.write("%d\n" % (populationSize))
    for i in range(0, populationSize):
    	theOutput.write("-1\n")
    	theOutput.write("%d\n" % (genomeLength))
    	for k in range(0, genomeLength):
	    line = "%.17f %.17f %.17f %.17f\n" % (gene[k], lowBound[k], highBound[k], sd[k])
	    theOutput.write(line)
    	line = "%s %s %s %s %s\n" % (fitness, parent1GenerationNumber, parent1Rank, parent2GenerationNumber, parent2Rank)
	theOutput.write(line)
    
    theOutput.close()
    

# program starts here

main()
   
